...
export default class ReferenceLine {
  ...
  bindEvent(): void {
    ...
    let box = options.container.nodeType
      ? doc
    ...
    fromEvent(box, "mousedown").subscribe((evt: any) => {
      ...
      if (ele.isRFItem) {
        _this.x = evt.clientX;
        _this.y = evt.clientY;
        subscriptionMove = fromEvent(box, "mousemove")
          .pipe(auditTime(options.auditTime))
          .subscribe((evt) => {
  ...
  initGrid(bcr: any) {
    ...
    let {
      cache: { boxLeft: baseX, boxTop: baseY },
    } = this.options;
    if (!grid) {
      return;
    }
  ...
  getPosition(): void {
    ...
    for (let v of items as any) {
      ...
      tempXLeft
        ? tempXLeft.push(position.length - 1)
        : (this.mapX[left] = [position.length - 1]);
      let tempXRight = this.mapX[right];
      tempXRight
        ? tempXRight.push(position.length - 1)
        : (this.mapX[right] = [position.length - 1]);
      ...
      if (this.options.center) {
        ...
        tempXCenterX
          ? tempXCenterX.push(position.length - 1)
          : (this.mapX[xCenter] = [position.length - 1]);
        let tempYCenterY = this.mapY[yCenter];
        tempYCenterY
          ? tempYCenterY.push(position.length - 1)
          : (this.mapY[yCenter] = [position.length - 1]);
      }

      if (this.options.hypotenuse) {
        this.mapH[wh]
          ? this.mapH[wh].push(position.length - 1)
          : (this.mapH[wh] = [position.length - 1]);
      }

      this.mapY[bcr.top]
        ? this.mapY[bcr.top].push(position.length - 1)
        : (this.mapY[Math.round(+bcr.top)] = [position.length - 1]);
      this.mapY[bcr.bottom]
        ? this.mapY[bcr.bottom].push(position.length - 1)
        : (this.mapY[Math.round(+bcr.bottom)] = [position.length - 1]);
    }
  ...
  getLine(x: number[], y: number[], isCenter: boolean): drawCoordinate {
    ...
    return <drawCoordinate>{
      vLine: tempV.length > limit ? vLine : null,
      hLine: tempH.length > limit ? hLine : null,
    };
  }
  ...
  autoMove(bcr: any, isMoveToRight: boolean, isMoveToBottom: boolean) {
    ...
    resultX = this.autoMoveProcess(
      [...this.gridX, ...Object.keys(this.mapX)],
      left,
      left + width,
      isMoveToRight
    );
    resultY = this.autoMoveProcess(
      [...this.gridY, ...Object.keys(this.mapY)],
      top,
      top + height,
      isMoveToBottom
    );
  ...
  move(evt: any, isSimulate: boolean): void {
    ...
    if (options.drag) {
      ...
      let maxLeft: number =
        range.width - (options.isMultiMove ? cache.widthMulti : p.width);
      let minTop: number = 0;
      let maxTop: number =
  ...
  createLine() {
    ...
    this.drawLine(
      [divide(add(right, left), 2), left, right],
      [divide(add(bottom, top), 2), top, bottom],
      true
    );
  }
  ...
  drawLine(x: number[], y: number[], isCenter: boolean): void {
    ...
    options.hLine &&
      temp.hLine &&
      this.drawHLine(temp.hLine.y, temp.hLine.start, temp.hLine.end);
    options.vLine &&
      temp.vLine &&
      this.drawVLine(temp.vLine.x, temp.vLine.start, temp.vLine.end);
  }
  ...
  getRange(): any {
    let options = this.options;
    let ele = options.range.nodeType
      ? doc.documentElement
      : doc.querySelector(options.range);
