let bookStore = [
  ...
  [
    {
      id: "1232adad12663822gb",
      name: "精通html",
      rating: 3.2,
      type: "前端技术类",
    },
    {
      id: "1232adad12263582ga",
      name: "移动端布局",
      rating: 2,
      type: "前端技术类",
    },
    {
      id: "1232adad12lmcx12ga",
      name: "js函数式编程",
      rating: 8,
      type: "前端技术类",
    },
    {
      id: "1232adad120kouy7ga",
      name: "数据结构与算法",
      rating: 7.3,
      type: "前端技术类",
    },
    {
      id: "1232adad123vmzliea",
      name: "css权威指南",
      rating: 6,
      type: "前端技术类",
    },
...
const curry = (fn, ...args) => {
  ...
  return (...args2) => {
    ...
    if (
      (argLens > 0 && _args.length < argLens) ||
      (argLens === 0 && args2.length !== 0)
    ) {
...
let curryReduce = partial(
  reduce,
  (total, book) => total + book.rating,
  undefined,
  0
); // 通过偏函数给 reduce 预设逻辑处理函数和初始值
...
const sortBy = (property, fn) => {
  return (a, b) => {
    if (typeof fn === "function") {
      return fn(a[property]) < fn(b[property])
        ? -1
        : fn(a[property]) > fn(b[property])
        ? 1
        : 0;
    } else {
